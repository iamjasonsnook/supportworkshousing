 ...
 */
function PdfStatus({ status, loaded, numPages, noun = 'booklet', fallbackUrl = REPORT_URL }) {
  ...
  if (status === 'error') {
    return (
      <div className="pdf-status">
        <p>
          We couldn't load the viewer. You can still{' '}
          <a href={fallbackUrl} target="_blank" rel="noopener noreferrer">open the PDF directly</a>.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="pdf-status">
      ...
      <p>Preparing the {noun}{numPages ? ` · ${pct}%` : '…'}</p>
    </div>
  );
}
